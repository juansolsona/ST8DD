...
static int T8DD_RPI_CheckGPIO_Exists(ST8DD_GPIO_Id Id) {
   ...
   RSP_GPIO_SYSFS_PATH "/" RSP_GPIO_INSTANCE_DIR "%.d/" RSP_GPIO_VALUE_FILE,
         (u32) Id);
...
static ST8DD_ErrorCodes ST8DD_InternalReleaseGPIO(ST8DD_RPI_GPIO_Handle *Handle) {
   ...
   if (-1 == fdGPIOExport) {
      LogFatal("Cannot open GPIO un-exports file (%s) ",
            RSP_GPIO_SYSFS_PATH RSP_GPIO_EXPORTS_PATH);
 ...
 * @return
 */
static ST8DD_ErrorCodes ST8DD_RPI_RequestGPIO(ST8DD_GPIO_Id Id,
   ...
   if (NULL == Handle) {
      ...
      if ((unsigned int) Id >= RSP_MAX_GPIO) {
         ...
         if (-1 != TestHandle) {
            ...
            O_WRONLY);
            if (-1 == fdGPIOExport) {
               LogFatal("Cannot open GPIO exports (%s) ",
                     RSP_GPIO_SYSFS_PATH RSP_GPIO_EXPORTS_PATH);
               RetValue = ST8DD_Err_InvalidPlatform;
            } else {
               bytes_written = snprintf(buffer, BUFFER_MAX, "%d", Id);
               if (bytes_written
                     != write(fdGPIOExport, buffer, bytes_written)) {
                  LogFatal("Cannot write to GPIO exports ");
                  RetValue = ST8DD_Err_InvalidPlatform;
               }
               ...
               *InternalHandle = malloc(sizeof(ST8DD_RPI_GPIO_Handle));
               if (NULL == *Handle) {
                  ...
                  if (-1 == (*InternalHandle)->fd_value) {
                     ...
                     char filename[RSP_GPIO_MAX_GPIO_FNAME];
                     snprintf(filename, sizeof(filename) - 1,
                           RSP_GPIO_SYSFS_PATH "/" RSP_GPIO_INSTANCE_DIR "%.d/" RSP_GPIO_DIRECTION_FILE,
                           (u32) Id);
 ...
 * @return
 */
static ST8DD_ErrorCodes ST8DD_RPI_SetDirectionGPIO(ST8DD_GPIO_Handle Handle,
   ...
   if (NULL == Handle) {
      ...
      if (ST8DD_Err_NoError == RetValue) {
         if (write(InternalHandle->fd_direction, DirectionStrPtr,
               strlen(DirectionStrPtr)) != strlen(DirectionStrPtr)) {
            RetValue = ST8DD_Err_InvalidPlatform;
 ...
 * @return
 */
static ST8DD_ErrorCodes ST8DD_RPI_GetDirectionGPIO(ST8DD_GPIO_Handle Handle,
      ST8DD_GPIO_Direction *Dir) {
   ST8DD_RPI_GPIO_Handle *InternalHandle = (ST8DD_RPI_GPIO_Handle *) Handle;
   ST8DD_ErrorCodes RetValue = ST8DD_Err_NoError;
   if (NULL == Handle) {
      ...
      char ChDir;
      if (read(InternalHandle->fd_direction, &ChDir, sizeof(ChDir))
            != sizeof(ChDir)) {
         RetValue = ST8DD_Err_InvalidPlatform;
         LogError("Cannot set to output pin");
      }
      ...
      default:
         RetValue = ST8DD_Err_InvalidPlatform;
         LogError("Invalid value on pin (%c)", ChDir);
 ...
 * @return
 */
static ST8DD_ErrorCodes ST8DD_RPI_SetValueGPIO(ST8DD_GPIO_Handle Handle,
      ST8DD_GPIO_Level Value) {
   const char *DirectionStrPtr;
   ST8DD_RPI_GPIO_Handle *InternalHandle = (ST8DD_RPI_GPIO_Handle *) Handle;
   ST8DD_ErrorCodes RetValue = ST8DD_Err_NoError;
   if (NULL == Handle) {
      ...
      if (ST8DD_Err_NoError == RetValue) {
         if (write(InternalHandle->fd_value, DirectionStrPtr,
               strlen(DirectionStrPtr)) != strlen(DirectionStrPtr)) {
            RetValue = ST8DD_Err_InvalidPlatform;
            LogError("Cannot set to output pin");
         }
 ...
 * @return
 */
static ST8DD_ErrorCodes ST8DD_RPI_GetValueGPIO(ST8DD_GPIO_Handle Handle,
      ST8DD_GPIO_Level *Value) {
   ST8DD_RPI_GPIO_Handle *InternalHandle = (ST8DD_RPI_GPIO_Handle *) Handle;
   ST8DD_ErrorCodes RetValue = ST8DD_Err_NoError;
   if (NULL == Handle) {
      ...
      char ChValue;
      if (read(InternalHandle->fd_value, &ChValue, sizeof(ChValue))
            != sizeof(ChValue)) {
         RetValue = ST8DD_Err_InvalidPlatform;
         LogError("Cannot set to output pin");
      }
      ...
      default:
         RetValue = ST8DD_Err_InvalidPlatform;
         LogError("Invalid value on pin (%c)", ChValue);
